...
- **Reprodução:** tríade C de três vozes é apresentada como `Tríade` e mostra `tônica, terça maior, quinta`; shell C7 com raiz, terça e sétima sem quinta é apresentada como `Shell`, mostra omissão de quinta e notas estruturais `tônica, terça maior, sétima menor`; Cmaj7 em quatro cordas adjacentes é lido como `Drop 2`; empilhamento C-F-Bb é lido como `Quartal`; shape E-G-B sobre Cmaj7 é lido como `Sem tônica`, omite tônica e mostra `terça maior, quinta, sétima maior`; cluster cromático/alterado consegue atingir tensão alta; shell Bb7b5 sem D agora mostra D como `3a (Impl.)` no braço de Materiais. Ainda falta levar condução interna e papéis por corda para explicações mais completas.
...
- **Correção recomendada:** propagar explicitamente condução interna e papéis por corda para explicações e estados da UI.
- **Testes necessários:** ampliar matriz para mais variações de acordes alterados e shapes rootless; cobertura inicial de tríade/shell/Drop 2/quartal/rootless/cluster existe em `writer-chord-reading-analysis.spec.ts` e de nota implícita em `local-material-note-roles.spec.ts`/`local-material-fretboard-notes.spec.ts`.
- **Confiança:** alta.
...
- **Observado:** resolvido parcialmente. `request_score` agora carrega `expiresAt`, o bridge poda mensagens expiradas, pedidos de sync pendentes são coalescidos, o snapshot/status carregam identidade de partitura (`scoreId/title`) e o adapter só carrega snapshots com `requestId` quando pertencem ao pedido ativo. O bridge emite `pluginSessionId`, direciona `request_score` à sessão de plugin mais recente e mutações do Writer também carregam `targetPluginSessionId` quando o status conhece um plugin ativo; a fila ainda não é segregada por score.
...
- **Reprodução:** sincronizar com plugin fechado, esperar timeout e abrir o plugin; pedido antigo não deve mais substituir o estado no dashboard, pois o `requestId` ativo foi limpo. Disparar dois syncs antes do polling deixa apenas o pedido mais recente na fila. Com duas instâncias, `request_score` novo e mutações do Writer podem ser marcados com a sessão de plugin ativa, mas ainda não há isolamento por score.
...
- **Progresso:** sync tardio deixa de permanecer indefinidamente na fila; parser MusicXML gera `metadata.scoreId`; `/api/v1/status` expõe a identidade da última partitura sincronizada e `pluginSessionId`; respostas de snapshot com `requestId` fora do pedido ativo são ignoradas antes de tocar o store; novo `request_score` substitui pedidos de sync anteriores ainda não consumidos; `/api/v1/plugin-session` entrega `pluginSessionId`, o QML envia esse ID no `/consume`, e o bridge marca `request_score` com `targetPluginSessionId` antes de enfileirar ou transmitir. O adapter consulta o status e inclui `targetPluginSessionId` em `INSERT_CHORD` quando disponível.
- **Correção recomendada:** filas por destino também para mutações, pending/cancelled requests no bridge e validação efetiva de `scoreId/sessionId` quando houver múltiplos plugins.
- **Testes necessários:** duas instâncias reais, troca de score, resposta fora de ordem rejeitada e mutações com alvo de partitura.
...
- **Observado:** resolvido parcialmente. O badge não promete mais disponibilidade do MuseScore quando só o bridge está conectado e já consome `pluginLastSeen`; inserção no Writer agora diferencia sucesso, cifra rejeitada, timeout, bridge offline e rejeição do plugin com mensagem visível. Existe vínculo de sessão de plugin para sync e mutações quando o status está disponível, mas ainda não há vínculo completo com score específico.
...
- **Progresso:** o protocolo tipado restringe mutations a `INSERT_CHORD`, remove `REPLACE/DELETE`, adiciona `commandId`, expiração e ACK. O plugin agora mantém um ledger curto por `commandId`: reentrega do mesmo comando reenvia o ACK anterior sem mutar a partitura novamente. O Writer preserva o `commandId` de uma falha e oferece retry idempotente da mesma operação. O bridge mantém um lease curto para mutações consumidas e remove o lease no ACK, podendo recolocar comandos não confirmados antes de expirarem. Ledger por score/instância ainda continua pendente.
- **Esperado:** retry seguro; ações desconhecidas rejeitadas; pareamento não recuperável por qualquer processo local.
- **Observado:** resolvido parcialmente. `/consume` remove a mutação da fila visível, mas agora a mantém em lease até ACK ou expiração do lease; reentrega com mesmo `commandId` não duplica a mutação no QML. Retry explícito do Writer reutiliza o mesmo ID quando há falha retornada pelo adapter, e a mutação pode carregar `targetPluginSessionId`; ainda não há ledger por score. `REPLACE_CHORD`/`DELETE_CHORD` não pertencem mais ao tipo público. `/plugin-session` entrega token sem autenticação para cliente local.
...
- **Correção recomendada:** ledger por score/instância e pareamento aprovado.
- **Testes necessários:** fault injection antes/depois da mutação em runtime real, replay por score e cada ação do protocolo.
