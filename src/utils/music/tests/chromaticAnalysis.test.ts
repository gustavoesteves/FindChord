...
console.log('\n🎵 Test 1 — Passing Diminished (Cmaj7 -> Bbdim7 -> Am7)');
{
  ...
  assert(bbDim.modal?.contextualFunction === 'PASSING_DIMINISHED', `contextualFunction is PASSING_DIMINISHED, got ${bbDim.modal?.contextualFunction}`);
  if (bbDim.modal?.chromaticAnalysis) {
    assert(bbDim.modal.chromaticAnalysis.type === 'PASSING_DIMINISHED', 'Type is PASSING_DIMINISHED');
...
console.log('\n🎵 Test 2 — Passing Diminished Descending (Fmaj7 -> Edim7 -> Dm7)');
{
  ...
  assert(eDim.modal?.contextualFunction === 'PASSING_DIMINISHED', `contextualFunction is PASSING_DIMINISHED, got ${eDim.modal?.contextualFunction}`);
  if (eDim.modal?.chromaticAnalysis) {
    assert(eDim.modal.chromaticAnalysis.type === 'PASSING_DIMINISHED', 'Type is PASSING_DIMINISHED');
...
console.log('\n🎵 Test 3 — Common-Tone & Neighbor Diminished (Cdim7 and C#dim7)');
{
  ...
  assert(cDim.modal?.contextualFunction === 'COMMON_TONE_DIMINISHED', `Cdim7 function is COMMON_TONE_DIMINISHED, got ${cDim.modal?.contextualFunction}`);
  if (cDim.modal?.chromaticAnalysis) {
    assert(cDim.modal.chromaticAnalysis.type === 'COMMON_TONE_DIMINISHED', 'Cdim7 type is COMMON_TONE_DIMINISHED');
  ...
  if (cSharp.modal?.chromaticAnalysis) {
    assert(cSharp.modal.chromaticAnalysis.type === 'NEIGHBOR_DIMINISHED', 'C#dim7 type is NEIGHBOR_DIMINISHED');
...
console.log('\n🎵 Test 4 — Chromatic Approach (F#maj7 -> Fmaj7)');
{
  ...
  assert(fSharp.modal?.contextualFunction === 'CHROMATIC_APPROACH', 'contextualFunction is CHROMATIC_APPROACH');
  if (fSharp.modal?.chromaticAnalysis) {
    assert(fSharp.modal.chromaticAnalysis.type === 'CHROMATIC_APPROACH', 'Type is CHROMATIC_APPROACH');
...
console.log('\n🎵 Test 5 — Tritone Substitution Precedence (Db7 -> Cmaj7)');
{
  ...
  assert(db7.secondary?.contextualFunction === 'TRITONE_SUBSTITUTION', 'Db7 is TRITONE_SUBSTITUTION');
  assert(db7.modal?.chromaticAnalysis === undefined, 'Db7 chromaticAnalysis is undefined');
}
...
console.log('\n🎵 Test 6 — Modal Borrowing Precedence (Dbmaj7 -> Cmaj7)');
{
  ...
  assert(dbmaj7.modal?.contextualFunction === 'MODAL_BORROWING', 'Dbmaj7 is MODAL_BORROWING');
  assert(dbmaj7.modal?.chromaticAnalysis === undefined, 'Dbmaj7 chromaticAnalysis is undefined');
}
