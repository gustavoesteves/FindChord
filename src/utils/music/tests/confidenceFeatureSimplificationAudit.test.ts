// Sprint F10-F.6 — Confidence Feature Simplification Audit (Score Gap)
// Run with: npx tsx src/utils/music/tests/confidenceFeatureSimplificationAudit.test.ts
...
function optimizeAblatedPlattScaling(
  ...
  for (let a = 2.0; a <= 25.0; a += 0.2) {
    for (let b = -15.0; b <= 0.0; b += 0.2) {
      ...
      const corrGoal = pearsonCorrelation(calibrated, goalAlignments);
...
function optimizeBaselinePlattScaling(
  rawConfidences: number[],
  normalizedScores: number[],
  frontierSizes: number[],
  hypervolumes: number[],
  ...
  for (let a = 2.0; a <= 25.0; a += 0.2) {
    for (let b = -15.0; b <= 0.0; b += 0.2) {
      ...
      const corrGeom = pearsonCorrelation(calibrated, geometries);
      const corrInfoGain = pearsonCorrelation(calibrated, informationGains);
      const corrGap = pearsonCorrelation(calibrated, scoreGaps);
...
async function main() {
  console.log('⚡ Iniciando F10-F.6 — Confidence Feature Simplification Audit...\n');
  ...
  console.log(`   ├─ Pesos Baseline: Gap=${optimizedW.scoreGapWeight.toFixed(4)} | Goal=${optimizedW.goalAlignmentWeight.toFixed(4)} | Geom=${optimizedW.geometryWeight.toFixed(4)} | Amb=${optimizedW.ambiguityWeight.toFixed(4)}`);

  const trainRawConfBaseline = trainRuns.map(r => 
  ...
  const baselinePlatt = optimizeBaselinePlattScaling(
    trainRawConfBaseline,
  ...
  console.log(`   ├─ Platt Baseline Otimizado: A=${baselinePlatt.a.toFixed(2)} | B=${baselinePlatt.b.toFixed(2)}`); 
  
  // 6. Otimização do Modelo Ablado (sem Score Gap)
  console.log('\n🧠 Treinando Modelo Ablado (sem Score Gap)...');
  const ablatedInputs = {
    goalAlignments: trainInputs.goalAlignments,
    geometries: trainInputs.geometries,
    informationGains: trainInputs.informationGains,
    successScores: trainInputs.successScores,
    frontierSizes: trainInputs.frontierSizes,
    hypervolumes: trainInputs.hypervolumes
  };
  const ablatedW = optimizeAblatedWeights(ablatedInputs);
  console.log(`   ├─ Pesos Ablados: Goal=${ablatedW.goalAlignmentWeight.toFixed(4)} | Geom=${ablatedW.geometryWeight.toFixed(4)} | Amb=${ablatedW.ambiguityWeight.toFixed(4)}`);

  const trainRawConfAblated = trainRuns.map(r => 
    (r.goalAlignmentRaw * ablatedW.goalAlignmentWeight) + 
  ...
  // 9. Bootstrap Stability Analysis (Criterion E)
  console.log('\n🔄 Executando Análise de Estabilidade Bootstrap (100 iterações)...');
  const B = 100;
  const N_train = trainRuns.length;
  const baselineWeightsHistory: number[][] = []; // [wGoal, wGeom, wAmb]
  const ablatedWeightsHistory: number[][] = [];  // [wGoal, wGeom, wAmb]

  // Seedable pseudo-random sampler for deterministic bootstrap results
  let seed = 42;
  ...
  for (let b = 0; b < B; b++) {
    const sampleRuns: RunData[] = [];
    for (let i = 0; i < N_train; i++) {
      const idx = Math.floor(random() * N_train);
      sampleRuns.push(trainRuns[idx]);
    }

    const sampleInputs = {
      scoreGaps: sampleRuns.map(r => r.scoreGapRaw),
      goalAlignments: sampleRuns.map(r => r.goalAlignmentRaw),
      geometries: sampleRuns.map(r => r.geometryRaw),
      informationGains: sampleRuns.map(r => r.informationGain),
      successScores: sampleRuns.map(r => r.successScore),
      frontierSizes: sampleRuns.map(r => r.frontierSize),
      hypervolumes: sampleRuns.map(r => r.hypervolume)
    };

    try {
      const bW = optimizeConfidenceWeights(sampleInputs);
      baselineWeightsHistory.push([bW.goalAlignmentWeight, bW.geometryWeight, bW.ambiguityWeight]);

      const aW = optimizeAblatedWeights({
        goalAlignments: sampleInputs.goalAlignments,
        geometries: sampleInputs.geometries,
        informationGains: sampleInputs.informationGains,
        successScores: sampleInputs.successScores,
        frontierSizes: sampleInputs.frontierSizes,
        hypervolumes: sampleInputs.hypervolumes
      });
      ablatedWeightsHistory.push([aW.goalAlignmentWeight, aW.geometryWeight, aW.ambiguityWeight]);
    } catch (err) {
      // Skip failed iterations in case of singular cases
    }
  }

  function computeCVForColumn(history: number[][], colIdx: number): { mean: number; std: number; cv: number } {
    const vals = history.map(row => row[colIdx]);
    const avg = vals.reduce((s, v) => s + v, 0) / vals.length;
    const std = stdDev(vals);
    const cv = avg > 0 ? std / avg : 0.0;
    return { mean: avg, std, cv };
  }

  const baseGoalStats = computeCVForColumn(baselineWeightsHistory, 0);
  const baseGeomStats = computeCVForColumn(baselineWeightsHistory, 1);
  const baseAmbStats = computeCVForColumn(baselineWeightsHistory, 2);

  const ablatedGoalStats = computeCVForColumn(ablatedWeightsHistory, 0);
  const ablatedGeomStats = computeCVForColumn(ablatedWeightsHistory, 1);
  const ablatedAmbStats = computeCVForColumn(ablatedWeightsHistory, 2);

  const cvBaseline = (baseGoalStats.cv + baseGeomStats.cv + baseAmbStats.cv) / 3;
  const cvAblated = (ablatedGoalStats.cv + ablatedGeomStats.cv + ablatedAmbStats.cv) / 3;

  console.log(`   ├─ Estabilidade dos pesos compartilhados (Goal, Geometry, Info Gain):`);
  console.log(`      ├─ Baseline: CV(Goal)=${baseGoalStats.cv.toFixed(4)} | CV(Geom)=${baseGeomStats.cv.toFixed(4)} | CV(Amb)=${baseAmbStats.cv.toFixed(4)} | Média CV=${cvBaseline.toFixed(4)}`);
  console.log(`      └─ Ablado:   CV(Goal)=${ablatedGoalStats.cv.toFixed(4)} | CV(Geom)=${ablatedGeomStats.cv.toFixed(4)} | CV(Amb)=${ablatedAmbStats.cv.toFixed(4)} | Média CV=${cvAblated.toFixed(4)}`);

  // 10. Avaliação das Regras de Decisão
  console.log('\n⚖️ Avaliando critérios de simplificação de Score Gap...');
  const critA = vifs[0] > 5;
  const critB = Math.abs(deltaBrier) < 0.002;
  const critC = Math.abs(deltaSpearman) < 0.03;
  const critD = normalizedShap[0] < 5.0;
  const critE = cvAblated <= cvBaseline * 1.05;

  console.log(`   ├─ Critério A (VIF > 5):                 ${critA ? '✅ Aprovado' : '❌ Rejeitado'} (VIF=${vifs[0].toFixed(4)})`);
  ...
  console.log(`   ├─ Critério D (SHAP Share < 5%):         ${critD ? '✅ Aprovado' : '❌ Rejeitado'} (Share=${normalizedShap[0].toFixed(2)}%)`);
  console.log(`   └─ Critério E (CV_ablated <= CV_base):   ${critE ? '✅ Aprovado' : '❌ Rejeitado'} (CV_ablated=${cvAblated.toFixed(4)} vs CV_base=${cvBaseline.toFixed(4)})`);

  const simplificationRecommended = critB && critC && critD && critE;
  console.log(`\n📢 PARECER DE SIMPLIFICAÇÃO:`);
  if (simplificationRecommended) {
    console.log('   🚀 STATUS: RECOMENDA-SE A REMOÇÃO DO SCORE GAP (SIMPLIFICAÇÃO AUTORIZADA)');
  } else {
    console.log('   🔒 STATUS: RECOMENDA-SE MANTER O SCORE GAP (CONTRIBUIÇÃO OPERACIONAL ATIVA)');
  }

  // 11. Gravar Relatório Markdown
  const reportPath = path.join('/Users/gustavoesteves/.gemini/antigravity-ide/brain/f6477136-2b69-47c8-8b1f-1aa05a2947ab', 'simplification_audit_report.md');
  const reportMd = `
# Relatório de Simplificação e Remoção do Score Gap (Sprint F10-F.6)

Este relatório consolida os resultados da auditoria de simplificação sobre a necessidade do fator **Score Gap** na confiança de recomendações do Find Chord.
...
| :--- | :---: | :---: | :--- |
| **Score Gap** | ${vifs[0].toFixed(4)} | ${partialCorrs[0].toFixed(4)} | ${Math.abs(partialCorrs[0]) > 0.05 ? 'Mantém sinal preditivo independente' : 'Sinal residual insignificante'} |
| **Goal Alignment** | ${vifs[1].toFixed(4)} | ${partialCorrs[1].toFixed(4)} | ${Math.abs(partialCorrs[1]) > 0.05 ? 'Mantém sinal preditivo independente' : 'Sinal residual insignificante'} |
| **Geometry** | ${vifs[2].toFixed(4)} | ${partialCorrs[2].toFixed(4)} | ${Math.abs(partialCorrs[2]) > 0.05 ? 'Mantém sinal preditivo independente' : 'Sinal residual insignificante'} |
| **Information Gain** | ${vifs[3].toFixed(4)} | ${partialCorrs[3].toFixed(4)} | ${Math.abs(partialCorrs[3]) > 0.05 ? 'Mantém sinal preditivo independente' : 'Sinal residual insignificante'} |
...
## 🧠 3. Estudo de Ablação (Modelo Completo vs Ablado sem Score Gap)

Comparamos o desempenho do recomendador em dados não vistos (Validation & Stress conjuntos) após re-treinar todos os pesos e Platt Scaling desconsiderando **Score Gap** ($w_{gap} = 0$):
...
* **Brier Score Delta ($\Delta BS$)**: ${deltaBrier.toFixed(6)} (Limite para simplificação: $|\Delta BS| < 0.002$)
* **Spearman Delta ($\Delta Spearman$)**: ${deltaSpearman.toFixed(4)} (Limite para simplificação: $|\Delta Spearman| < 0.03$)
...
## 🔄 5. Estabilidade dos Pesos (Bootstrap Stability Analysis - 100 Iterações)

Avaliamos a dispersão dos pesos otimizados (Coefficient of Variation) sob reamostragem com reposição para determinar se a remoção do Score Gap melhora ou prejudica a estabilização matemática do modelo:

| Métrica de Estabilidade | Modelo Baseline (4-Features) | Modelo Ablado (3-Features) | Delta de Variação |
| :--- | :---: | :---: | :---: |
| **CV(Goal Alignment)** | ${baseGoalStats.cv.toFixed(4)} | ${ablatedGoalStats.cv.toFixed(4)} | ${(ablatedGoalStats.cv - baseGoalStats.cv).toFixed(4)} |
| **CV(Geometry)** | ${baseGeomStats.cv.toFixed(4)} | ${ablatedGeomStats.cv.toFixed(4)} | ${(ablatedGeomStats.cv - baseGeomStats.cv).toFixed(4)} |
| **CV(Information Gain)** | ${baseAmbStats.cv.toFixed(4)} | ${ablatedAmbStats.cv.toFixed(4)} | ${(ablatedAmbStats.cv - baseAmbStats.cv).toFixed(4)} |
| **Média do CV dos Pesos** | ${cvBaseline.toFixed(4)} | ${cvAblated.toFixed(4)} | ${(cvAblated - cvBaseline).toFixed(4)} |

> [!TIP]
> Um valor de **CV (Coefficient of Variation)** menor indica maior estabilidade na convergência matemática dos pesos.
> Se a média do CV do modelo ablado for menor ou igual à do baseline ($CV_{\text{ablated}} \le CV_{\text{baseline}} \cdot 1.05$), o critério de estabilidade é atendido.

---

## ⚖️ 6. Avaliação do Parecer de Simplificação (Regra de Decisão)

Avaliamos os cinco critérios estatísticos pré-estabelecidos para decidir sobre a simplificação (remoção) de **Score Gap**:

* **[ ${critA ? 'Sim' : 'Não'} ] Critério A**: VIF do Score Gap é superior a 5.0 (Valor obtido: **${vifs[0].toFixed(4)}**).
...
* **[ ${critD ? 'Sim' : 'Não'} ] Critério D**: Importância relativa SHAP de Score Gap é inferior a 5.0% (Valor obtido: **${normalizedShap[0].toFixed(2)}%**).
* **[ ${critE ? 'Sim' : 'Não'} ] Critério E**: A estabilidade bootstrap do ablado é igual ou superior ao baseline ($CV_{\text{ablated}} \le CV_{\text{baseline}} \cdot 1.05$) (Valores: **${cvAblated.toFixed(4)}** vs **${cvBaseline.toFixed(4)}**).

### 📢 PARECER FINAL:
> [!${simplificationRecommended ? 'WARNING' : 'NOTE'}]
> **${simplificationRecommended 
      ? 'RECOMENDA-SE A REMOÇÃO DO SCORE GAP (SIMPLIFICAÇÃO AUTORIZADA)' 
      : 'RECOMENDA-SE MANTER O SCORE GAP (CONTRIBUIÇÃO OPERACIONAL ATIVA)'}**
> 
> ${simplificationRecommended 
      ? 'Score Gap atendeu a todos os critérios operacionais de simplificação. Sua remoção não prejudica o erro de calibração nem o rank de ordenação do recomendador, além de reduzir a variabilidade dos pesos sob bootstrap (CV). Portanto, recomenda-se consolidar o modelo com base nos três pilares fundamentais: Goal Alignment, Geometry e Information Gain.' 
      : 'Score Gap não atendeu a todos os critérios de simplificação, indicando que sua remoção causa impactos significativos na qualidade do recomendador.'}
`;

  fs.writeFileSync(reportPath, reportMd.trim(), 'utf-8');
  console.log(`\n📝 Relatório de Simplificação persistido em: ${reportPath}`);
  console.log('\n🎉 SPRINT F10-F.6 EXECUTADA COM SUCESSO!');
}
