...
console.log('\n🎵 Test 1 — Backdoor Cadence Precedence (Bb7 -> Cmaj7)');
{
  ...
  assert(bb7Chord.modal?.contextualFunction !== 'MODAL_BORROWING', 'Bb7 NOT classified as MODAL_BORROWING');
  assert(bb7Chord.modal?.modalBorrowing === undefined, 'Bb7 modalBorrowing is undefined');
}
...
console.log('\n🎵 Test 2 — Secondary Dominant Precedence (Cmaj7 -> A7 -> Dm7 -> G7)');
{
  ...
  assert(a7Chord.secondary?.contextualFunction === 'SECONDARY_DOMINANT', 'A7 is SECONDARY_DOMINANT');
  assert(a7Chord.modal?.modalBorrowing === undefined, 'A7 modalBorrowing is undefined (NOT modal borrowing)');
}
...
console.log('\n🎵 Test 3 — Napolitano bII (Dbmaj7 -> PHRYGIAN)');
{
  ...
  if (dbmaj7.modal?.modalBorrowing) {
    assert(dbmaj7.modal.modalBorrowing.sourceMode === 'PHRYGIAN', 'Dbmaj7 sourceMode is PHRYGIAN');
...
console.log('\n🎵 Test 4 — Picardy Third (Cm -> Ab -> G7 -> C)');
{
  ...
  assert(lastChord.modal?.contextualFunction === 'MODAL_BORROWING', 'Last chord is MODAL_BORROWING');
  if (lastChord.modal?.modalBorrowing) {
    assert(lastChord.modal.modalBorrowing.sourceMode === 'IONIAN', 'Last chord sourceMode is IONIAN');
...
console.log('\n🎵 Test 5 — Mixolydian vs Aeolian (Bbmaj7 vs Bb7)');
{
  ...
  assert(bbmaj7.modal?.contextualFunction === 'MODAL_BORROWING', 'Bbmaj7 is MODAL_BORROWING');
  if (bbmaj7.modal?.modalBorrowing) {
    assert(bbmaj7.modal.modalBorrowing.sourceMode === 'MIXOLYDIAN', 'Bbmaj7 sourceMode is MIXOLYDIAN');
  ...
  assert(bb7.modal?.contextualFunction === 'MODAL_BORROWING', 'Bb7 is MODAL_BORROWING');
  if (bb7.modal?.modalBorrowing) {
    assert(bb7.modal.modalBorrowing.sourceMode === 'AEOLIAN', 'Bb7 sourceMode is AEOLIAN');
