// Sprint 10A — Tonal Hierarchy & Tonal Region Tree Tests (Updated for HarmonicRegion)
// Run with: npx tsx src/utils/music/tests/tonalHierarchy.test.ts

import { analyzeProgression, buildHarmonicRegionTree, getRegionRank } from '../analysis/functionalAnalysis';
import type { HarmonicRegion, TonalCenter } from '../analysis/models/FunctionalAnalysis';
...
// Helper para criar mock de HarmonicRegion
function makeMockRegion(
  root: string,
  mode: 'MAJOR' | 'MINOR',
  type: 'TONICIZATION' | 'REGIONAL_SHIFT' | 'ESTABLISHED_MODULATION' | 'MODAL_AXIS',
  isHomeKey: boolean
): HarmonicRegion {
  const isMajor = mode === 'MAJOR';
  const modalMode = isMajor ? 'IONIAN' : 'AEOLIAN';
  return {
    state: { root, mode: modalMode },
    baseCenter: makeMockCenter(root, mode),
    startIndex: 0,
    endIndex: 0,
    type,
    isHomeKey,
...
console.log('\n🎵 Caso 1 — Diatonic Stable (Cmaj7 -> Am7 -> Dm7 -> G7 -> Cmaj7)');
{
  ...
  if (analysis.regionTree) {
    ...
    assert(root.region.baseCenter.root === 'C' && root.region.baseCenter.mode === 'MAJOR', 'Root region is C MAJOR');
...
console.log('\n🎵 Caso 2 — Unit Test: Tonicização Sob Home Key (C Major -> D Minor Tonicization)');
{
  ...
  const root = buildHarmonicRegionTree([home, tonicization]);
  
  assert(root !== null, 'Tree built successfully');
  if (root) {
    assert(root.region.baseCenter.root === 'C', 'Root is C');
    ...
    assert(child.region.baseCenter.root === 'D' && child.region.baseCenter.mode === 'MINOR', 'Child is D Minor');
...
console.log('\n🎵 Caso 3 — Unit Test: Hierarquia Complexa (C -> Dm -> Am -> Em -> C)');
{
  ...
  const root = buildHarmonicRegionTree([reg0, reg1, reg2, reg3, reg4]);
  ...
  if (root) {
    ...
    if (root.children.length === 3) {
      ...
      assert(child0.region.baseCenter.root === 'D' && child0.region.baseCenter.mode === 'MINOR', 'First child is D Minor');
      assert(child0.parent === root, 'D Minor parent is Root');
      assert(child0.children.length === 0, 'D Minor has 0 children');
      
      assert(child1.region.baseCenter.root === 'A' && child1.region.baseCenter.mode === 'MINOR', 'Second child is A Minor');
      ...
      if (child1.children.length === 1) {
        ...
        assert(grandChild.region.baseCenter.root === 'E' && grandChild.region.baseCenter.mode === 'MINOR', 'A Minor child is E Minor');
        assert(grandChild.parent === child1, 'E Minor parent is A Minor');
        assert(grandChild.children.length === 0, 'E Minor has 0 children');
      }
      
      assert(child2.region.baseCenter.root === 'C' && child2.region.baseCenter.mode === 'MAJOR', 'Third child is C Major Return');
...
console.log('\n🎵 Caso 4 — JSON Serialization Safety');
{
  ...
  const tree = buildHarmonicRegionTree([reg0, reg1]);
