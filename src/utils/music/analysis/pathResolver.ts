...
export function resolveGlobalPath(
  ...
  for (let j = 0; j < columns[0].length; j++) {
    ...
    if (initialTonalCenter) {
      if (state.key.root === initialTonalCenter.root && state.key.mode === initialTonalCenter.mode) {
        startMultiplier = 1.0;
  ...
  for (let i = 1; i < N; i++) {
    ...
    for (let j = 0; j < curCol.length; j++) {
      ...
      for (let k = 0; k < prevCol.length; k++) {
        ...
        const keyMultiplier = getKeyTransitionMultiplier(prevState.key, curState.key);
  ...
  for (let i = 1; i < N; i++) {
    ...
    const isSameKey = prevState.key.root === curState.key.root && prevState.key.mode === curState.key.mode;
    const keyMult = getKeyTransitionMultiplier(prevState.key, curState.key);
