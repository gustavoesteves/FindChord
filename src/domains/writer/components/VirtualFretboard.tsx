...
export const VirtualFretboard: React.FC = () => {
  ...
  const { activeScale } = useChordStore();
      ...
      <div className="w-full overflow-x-auto rounded-xl border border-zinc-800/80 glass-panel p-4 shadow-2xl relative select-none">
        ...
        <div className="min-w-[1360px] relative fretboard-wood">
          <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
            ...
            <defs>
              <linearGradient id="fretboard-wood-grad-writer" x1="0%" y1="0%" x2="0%" y2="100%">
                <stop offset="0%" stopColor="#252528" />
                <stop offset="50%" stopColor="#1a1a1c" />
                <stop offset="100%" stopColor="#131315" />
              </linearGradient>
              <linearGradient id="nut-grad-writer" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="#444" />
                <stop offset="50%" stopColor="#777" />
                <stop offset="100%" stopColor="#222" />
              </linearGradient>
              <radialGradient id="inlay-glow-writer" cx="50%" cy="50%" r="50%">
            ...
            <rect 
              ...
              height={height - 20} 
              fill="url(#fretboard-wood-grad-writer)"
              rx="4"
            />
            ...
            {DOUBLE_FRET_MARKERS.map(fret => {
              ...
              return (
                <g key={`inlay-double-b-${fret}`}>
                  <circle cx={x} cy={height / 2 - 40} r="5.5" fill="url(#inlay-glow-writer)" />
                  <circle cx={x} cy={height / 2 + 40} r="5.5" fill="url(#inlay-glow-writer)" />
                </g>
              );
            })}
              ...
              return (
                <g key={`marks-row-b-${stringIdx}`}>
                  ...
                  {Array.from({ length: fretCount + 1 }).map((_, fret) => {
                    ...
                    // --- 1. MODO SCALE OVERLAY (Notas da escala selecionada acesas) ---
                    ...
                    // --- 2. MODO DE ENTRADA LIVRE (Desenha apenas a nota ativada fisicamente) ---
                    if (isFretted) {
