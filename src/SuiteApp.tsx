...
import { FileCode, CheckSquare, Map, Link, Link2Off, RefreshCw } from "lucide-react";

export default function SuiteApp() {
  const [currentTab, setCurrentTab] = useState<"playground" | "builder" | "explorer">("builder");
  const [connStatus, setConnStatus] = useState<ConnectionStatus>("disconnected");

  useEffect(() => {
    // Inicia conexão automática com o bridge local
    musescoreAdapter.connect();

    // Se inscreve nos eventos de status
  ...
  return (
    <div className="min-h-screen bg-stage-lights flex flex-col transition-colors duration-300">
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-4 py-6 md:py-8 flex flex-col gap-6 md:gap-8 box-border">
        ...
        {/* Barra de Navegação do Suite de Integração MuseScore */}
        <div className="w-full flex flex-col md:flex-row items-center justify-between gap-4 border-b border-zinc-800 pb-4">
          <div className="flex flex-col gap-0.5">
            <h1 className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500 tracking-tight uppercase">
              Find Chord Compose Suite
            </h1>
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">
              Suíte de Transcrição e Integração MuseScore v1.0
            </p>
          </div>

          <div className="flex flex-col sm:flex-row items-center gap-3">
            {/* Status do Host (MuseScore) */}
            ...
            <div className="flex items-center gap-1.5 p-1 bg-zinc-950/60 rounded-xl border border-zinc-850">
              <button
                ...
                className={`px-4 py-2 text-xs font-black rounded-lg transition-all flex items-center gap-1.5 cursor-pointer ${currentTab === "playground" ? "bg-purple-600 text-white shadow-lg shadow-purple-950/30" : "text-zinc-400 hover:text-zinc-200"}`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-black rounded-lg transition-all flex items-center gap-1.5 cursor-pointer ${currentTab === "builder" ? "bg-purple-600 text-white shadow-lg shadow-purple-950/30" : "text-zinc-400 hover:text-zinc-200"}`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-black rounded-lg transition-all flex items-center gap-1.5 cursor-pointer ${currentTab === "explorer" ? "bg-purple-600 text-white shadow-lg shadow-purple-950/30" : "text-zinc-400 hover:text-zinc-200"}`}
              >
      ...
      <footer className="w-full border-t border-zinc-850/60 bg-zinc-950/60 backdrop-blur-md py-4 text-center mt-auto">
        <p className="text-[11px] text-zinc-500 font-semibold tracking-wider uppercase">
          Find Chord Compose Suite © 2026 • Ferramenta de Produtividade Harmônica
        </p>
      </footer>
    </div>
  );
